#!/usr/bin/env python
# -*- coding:utf-8 -*-
# @FileName  :DSAN-bearing0.py
# @Time      :2021/2/26 17:02
# @Author    :Xue Liu
...
def train(epoch, model):
    ...
    for i in range(1, num_iter):
        ...
        if soft_label:
            ...
            soft_label_for_batch = mmdb.ret_soft_label(label_target, soft_labels)
            soft_label_for_batch = soft_label_for_batch.cuda()
            soft_label_for_batch = Variable(soft_label_for_batch)
            tgt_output_cl, _ = model(data_target)
            output_cl_score = F.softmax(tgt_output_cl / temperature, dim=1)
            loss_soft = - (torch.sum(soft_label_for_batch * torch.log(output_cl_score))) / float(
                output_cl_score.size(0))

            model.train()

            loss = loss_cls + param * lambd * loss_mmd + nu * loss_soft
